...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    const animateFace = (e: any) => {
      ...
      const caretPos = emailCoords.x + (email.length * 8); 
      ...
      const eyeLAngle = getAngle(eyeLCoords.x, eyeLCoords.y, emailCoords.x + caretPos, emailCoords.y + 25);
      const eyeRAngle = getAngle(eyeRCoords.x, eyeRCoords.y, emailCoords.x + caretPos, emailCoords.y + 25);
      const noseAngle = getAngle(noseCoords.x, noseCoords.y, emailCoords.x + caretPos, emailCoords.y + 25);
      const mouthAngle = getAngle(mouthCoords.x, mouthCoords.y, emailCoords.x + caretPos, emailCoords.y + 25);
    ...
    if (emailInput) {
      emailInput.addEventListener("input", animateFace);
      emailInput.addEventListener("focus", animateFace);
    ...
    return () => {
      if (emailInput) {
        emailInput.removeEventListener("input", animateFace);
        emailInput.removeEventListener("focus", animateFace);
      }
    };
  }, [email]);
  ...
  return (
    ...
    <div className="min-h-screen flex items-center justify-center bg-black text-white px-4 relative overflow-hidden">
      ...
      <div className="absolute top-0 left-0 w-full h-full overflow-hidden z-0 pointer-events-none">
        <div className="absolute top-[-10%] right-[-10%] w-[400px] h-[400px] bg-purple-900/30 rounded-full blur-[100px]" />
        <div className="absolute bottom-[-10%] left-[-10%] w-[400px] h-[400px] bg-blue-900/30 rounded-full blur-[100px]" />
      </div>

      {/* Login Card (Dark Mode) */}
      <form onSubmit={handleSubmit} className="w-full max-w-[400px] bg-gray-900 p-8 rounded-2xl border border-gray-800 relative z-10 shadow-2xl">
        
        {/* YETI CONTAINER (Dark Background Circle) */}
        <div className="w-[200px] h-[200px] mx-auto -mt-24 mb-6 relative rounded-full bg-black border-4 border-gray-800 overflow-hidden shadow-[0_0_20px_rgba(168,85,247,0.2)]">
          <div className="svgContainer" ref={svgRef} style={{ width: '100%', height: '100%' }}>
            <svg className="mySVG" viewBox="0 0 200 200">
              ...
              {/* Background of Yeti is now Black to match hole */}
        ...
        <h2 className="text-3xl font-extrabold text-center text-transparent bg-clip-text bg-gradient-to-r from-purple-400 to-blue-500 mb-6">
          Welcome Back
        </h2>
        ...
        <div className="space-y-5">
          ...
          <button 
            ...
            disabled={loading}
            className="w-full py-4 mt-2 bg-gradient-to-r from-purple-600 to-blue-600 hover:from-purple-700 hover:to-blue-700 text-white font-bold text-lg rounded-xl shadow-lg transition-transform active:scale-95"
          >
        ...
        <div className="mt-6 text-center">
          <Link href="/register" className="text-sm font-bold text-gray-500 hover:text-purple-400 transition-colors">
